...
- **创建抽奖交互重做为「一问一答」的一行式**
  - `创建抽奖` 先输出**创建格式与参考用例**，用户的**下一条消息**即为全部内容：`奖品 开奖时间 [加入口令]`
  - 回复不符合规则（不足两段、开奖时间不合法）或回复其他内容 → **直接取消创建**，不再逐步追问；
    明确回复 `q` / `取消` 或超时同样取消
  - 标题、描述、中奖人是否可重复不再提问，需要时用 `-t` / `-d` / `-r` 覆盖
  - 支持位置参数完全跳过提示：`抽奖 add 显卡*1 09-15-20-00 参加`
  - 奖品支持 `|` 分隔多个、解析忽略空行；口令允许含空格并放在最后
