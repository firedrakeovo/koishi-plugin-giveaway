...
- **开奖提醒改为控制台配置**：新增 `remind.beforeEnd`（可填多行「开奖前 N」偏移，如 `0-0-0-1-0` = 开奖前 1 小时、
  `0-0-0-0-30` = 开奖前 30 分钟），新建抽奖时按每个偏移排一个提醒任务，到点在抽奖所在的全部频道播报
  （倒计时按频道语言本地化）
  - 默认阶梯 **开奖前 10 分钟 / 1 小时 / 5 小时 / 1 天 / 3 天**（`0-0-0-0-10`、`0-0-0-1-0`、`0-0-0-5-0`、`0-0-1-0-0`、`0-0-3-0-0`）：
    偏移与抽奖时长无关，已经过去的偏移会自动跳过，所以 1 小时的短场次只提醒 10 分钟那档、7 天的长场次用满整条阶梯

### Removed 移除

- **手动提醒器交互全部下线**：`创建提醒器` / `提醒器列表` / `启用提醒器` / `禁用提醒器` / `删除提醒器` /
  `抽奖提醒` 六条指令不再注册（指令一览里也不再出现），提醒统一由控制台 `remind.beforeEnd` 配置；
  旧版提醒器相关的 i18n 文案、数据表与事件仅内部兼容保留，不影响已有数据加载
