...
- **多语言 / 多时区**：简体中文 / English / Deutsch **三套语言包全部翻译齐全**（`语言` 指令可按用户或按频道切换），用户可自设时区
...
- **参与条件**（模板最后一行）：直接显示控制台当前配置，创建者有三种选择 —— 不改动 = 沿用控制台配置；删空 = 这个抽奖不限制；改写 = 自定义（写法 `等级≥40 活跃≥1 连续≥7 标识=群聊炽焰,龙王`，不需要的项删掉即可）。文本表达不了的口径（多标识判定、龙王口径）只在控制台配置、对所有抽奖生效
- **当前生效的参与条件会显示在 `抽奖详情` 里**（per-roll 优先，其次是控制台配置；没有任何限制时显示「不限」），参加者被拒之前就能看到门槛
...
| | `honorMode` | 勾选多个标识时是「满足任意一个」还是「必须全部满足」，默认任意一个（**仅控制台配置**，创建模板里不含此项） |
...
- 控制台里「互动标识」若留了一行空值，它会以 `null` 存进配置：插件**按无效值忽略**，并在启动日志里告警说明当前实际生效的标识（三语候选项都是 `required`，所以空行在控制台上会显示为空行，不会伪装成已选中的某个标识）；
