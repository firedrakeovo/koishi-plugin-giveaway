...
- **创建抽奖只需 1~2 步**：交互式提问压缩为「奖品 → 开奖时间与加入口令」（后两项可一条消息一起回答），标题/描述/开奖类型走默认值；也给熟手准备了**完全非交互**的一条指令写法
...
```
> 抽奖 add
< 请输入你要抽的奖品：格式：奖品名称*数量，每行一个
> 显卡*1
< 请回复开奖时间与加入口令（用空格分隔，回复 n 表示该项不要）
> 09-15-20-00 参加
< 创建抽奖成功，编号为 5645
```
...
抽奖 add -t "双十一抽奖" -d "满 40 级可参加" -r 显卡*1 n n
```

- 省略哪个字段就问哪个字段；`q` 取消，`undo` 回上一步
- `-t` 标题、`-d` 描述、`-r` 允许中奖人重复、`-n` 快速模式（只问奖品）
...
| `giveaway.add` | 创建抽奖 | 创建抽奖：`抽奖 add` 进入两步问答，或直接 `抽奖 add <奖品> [时间] [口令]` |
