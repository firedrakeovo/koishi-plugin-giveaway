...
- **创建抽奖只需一问一答**：`创建抽奖` 先给出格式与参考用例，你回复**一条消息**（`奖品 开奖时间 [口令]`）即可；不符合规则或回复其他内容则直接取消，不会反复追问。熟手也可用一条指令跳过提示
...
```
> 创建抽奖
< 请按下面的格式回复一条消息来创建抽奖（回复其他内容会取消本次创建）：
<   奖品：名称*数量，多个奖品用 | 分隔，如 显卡*1|鼠标*2
<   开奖时间：年-月-日-时-分，填 n 表示不自动开奖
<   加入口令：可省略，填 n 表示不用口令
<   参考用例：显卡*1 09-15-20-00 参加
> 显卡*1 09-15-20-00 参加
< 创建抽奖成功，编号为 5645
```

熟手也可以跳过提示，直接带参数创建（零问答）：

```
抽奖 add 显卡*1 09-15-20-00 参加          # 奖品 / 开奖时间 / 加入口令
抽奖 add 显卡*1|鼠标*2 n n                # 不自动开奖、不用口令
抽奖 add -t "双十一抽奖" -d "满 40 级可参加" -r 显卡*1 n 参加
```

规则：

- 回复必须至少给出「**奖品 + 开奖时间**」两段（时间填 `n` 表示不自动开奖）；不足两段或时间不合法 → **直接取消创建**，不会追问，重新发送 `创建抽奖` 即可
- 奖品名不要含空格；多个奖品用 `|` 分隔，数量写成 `名称*数量`
- 加入口令放在最后，允许含空格、可省略（缺省表示不用口令）
- `-t` 标题、`-d` 描述、`-r` 允许同一个中奖人重复被抽中（默认为不允许）
